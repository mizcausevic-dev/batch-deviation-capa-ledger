...
public static class RenderService
{
    ...
    public static string Overview() => Layout(
        ...
        <section class="section">
          <div class="sh"><h2>Operator Snapshot</h2><div class="note">biotech · deviation · capa</div></div>
          <div class="kpis">
            {{Metric("2", "batch snapshots", "Synthetic deviation and CAPA bundles across active quality lanes.", "cyan")}}
            {{Metric("1", "current bundles", "Only one bundle is fresh enough to trust without refresh.", "green")}}
            {{Metric("6", "gaps", "Control deviations across root-cause packets, ownership, effectiveness, recurrence, training, and final disposition.", "plum")}}
            {{Metric("4", "blocking gaps", "Disposition-blocking issues still need closure before batch handoff.", "red")}}
            {{Metric("3", "deviation risks", "Intake, recurrence, and retraining continuity still need repair.", "amber")}}
            {{Metric("3", "capa risks", "Ownership, effectiveness, and disposition posture is still incomplete.", "red")}}
          </div>
        </section>
        <section class="section">
          <div class="sh"><h2>Why this lane matters</h2><div class="note">biotech / diagnostics / csharp</div></div>
          <div class="stack">
            <div class="src"><div class="src-name">quality confidence</div><div class="src-tit">Stop weak CAPA packets from reaching batch disposition</div><p>Biotech teams need one board where root-cause evidence, CAPA ownership, effectiveness checks, recurrence pressure, retraining proof, and final disposition stay readable together.</p></div>
            <div class="src"><div class="src-name">founder edge</div><div class="src-tit">Enterprise workflow depth, not biotech keyword stuffing</div><p>This fits the Kinetic Gain pattern: routing, evidence, approvals, and operator-safe remediation posture for quality-critical work.</p></div>
    ...
    public static string BatchLane() => Layout(
        "Batch Deviation CAPA Ledger — Batch Lane",
        "/batch-lane",
        $$"""
        <section class="section">
          <div class="sh"><h2>Batch Lane</h2><div class="note">owner · focus · next action</div></div>
          <table class="ttbl">
            <thead><tr><th>Lane</th><th>Owner</th><th>Status</th><th>Focus</th><th>Next action</th></tr></thead>
            <tbody>
              {{string.Join("", SampleData.BatchLane.Select(lane => $$"""
                <tr>
    ...
    public static string DeviationFindings() => Layout(
        "Batch Deviation CAPA Ledger — Deviation Findings",
        "/deviation-findings",
        $$"""
        <section class="section">
          <div class="sh"><h2>Deviation Findings</h2><div class="note">severity · owner · subject</div></div>
          <table class="ttbl">
    ...
    public static string CapaPosture() => Layout(
        "Batch Deviation CAPA Ledger — CAPA Posture",
        "/capa-posture",
        $$"""
        <section class="section">
          <div class="sh"><h2>CAPA Posture</h2><div class="note">packet readiness · blocker · timing</div></div>
          <div class="board">
            {{string.Join("", SampleData.CapaPackets.Select(packet => $$"""
              <article class="pcard">
                <div class="ptop">
                ...
                <ul class="check">
                  ...
                  <li>{{packet.LaunchWindowHours}} hours to the next quality checkpoint</li>
    ...
    public static string Verification() => Layout(
        "Batch Deviation CAPA Ledger — Verification",
        "/verification",
        $$"""
        <section class="section">
          <div class="sh"><h2>Verification</h2><div class="note">operator-safe claims only</div></div>
          <div class="stack">
            {{VerificationCard("This repo uses synthetic deviation and CAPA evidence only; no live patient, lab, or proprietary biotech data is published.")}}
            {{VerificationCard("The control plane is rooted in root-cause packets, CAPA ownership, effectiveness checks, recurrence pressure, operator retraining, and final disposition mechanics.")}}
    ...
    public static string Docs() => Layout(
        "Batch Deviation CAPA Ledger — Docs",
        "/docs",
        $$"""
        <section class="section">
          <div class="sh"><h2>Docs</h2><div class="note">routes · runbook · api</div></div>
          <div class="stack">
            <div class="src"><div class="src-name">routes</div><div class="src-tit">Public proof surface</div><p><code>/</code>, <code>/batch-lane</code>, <code>/deviation-findings</code>, <code>/capa-posture</code>, <code>/verification</code>, <code>/docs</code></p></div>
            <div class="src"><div class="src-name">api</div><div class="src-tit">Structured payloads</div><p><code>/api/dashboard/summary</code>, <code>/api/batch-lane</code>, <code>/api/deviation-findings</code>, <code>/api/capa-posture</code>, <code>/api/verification</code>, <code>/api/sample</code></p></div>
            <div class="src"><div class="src-name">runbook</div><div class="src-tit">Local execution</div><p><code>dotnet run --project src/BatchDeviationCapaLedger.Api -- --demo</code> prints the same quality posture used by the public proof surface.</p></div>
          </div>
        </section>
        """
    );
    ...
    public static string Sample() => JsonSerializer.Serialize(new
    {
        ...
        batchLane = SampleData.BatchLane,
        deviationFindings = SampleData.Payload.Gaps,
        capaPosture = SampleData.CapaPackets,
    ...
    public static string Layout(string title, string active, string body)
    {
        ...
        {
            Nav("/", "Overview"),
            Nav("/batch-lane", "Batch Lane"),
            Nav("/deviation-findings", "Deviation Findings"),
            Nav("/capa-posture", "CAPA Posture"),
        ...
        <body>
          <div class="wrap">
            <div class="topbar">
              <div class="left">Kinetic Gain · Batch Deviation CAPA Ledger</div>
              <div>synthetic deviation packets · no patient or proprietary biotech data</div>
            </div>
            <div class="herorow">
              <section class="hero">
                <div class="chiprow">
                  <span class="meta-chip">Biotech / Diagnostics quality stack</span>
                  <span class="meta-chip">C# · deviation and CAPA operator proof</span>
                  <span class="meta-chip">Hosted preview planned · Embedded by engagement</span>
                </div>
                <h1>Batch deviations, CAPA ownership, and disposition posture that stay operator-readable.</h1>
                <p>This control plane turns synthetic deviation and CAPA exports into one review surface: root-cause packet continuity, accountable ownership, effectiveness checks, recurrence pressure, retraining evidence, and final batch disposition completeness before downstream handoff.</p>
                <div class="navrow">{{{navHtml}}}</div>
              </section>
              <aside class="side">
                <div class="bluf"><div class="lbl">Commercial front door</div><p><strong>Deviation routing, CAPA posture, and batch disposition operations for biotech quality teams.</strong><br />The free surface is buyer-readable proof; the commercial path is an embedded deviation and CAPA module for regulated quality workflows.</p></div>
                <div class="corr"><div class="lbl">Proof layer</div><p><strong>.NET API + static operator shell.</strong><br />The repo models root-cause evidence, CAPA ownership, effectiveness checks, recurrence pressure, and final disposition without pretending to be a live tenant integration.</p></div>
                <div class="corr"><div class="lbl">Why it matters</div><p>Biotech quality teams need CAPA ownership and batch disposition in one lane, not scattered QA docs and spreadsheet trackers.</p></div>
              </aside>
            </div>
            {{{body}}}
            <div class="footer">
